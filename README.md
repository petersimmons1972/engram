...
| `memory_recall`           | Search across keyword, semantic, and graph layers. Returns summaries by default (`detail="summary"`) |
| `memory_list`             | Browse recent memories, filtered by type, tag, or importance. Summary mode by default               |
...
| `memory_status`           | View stats: memory count, chunks, summarization progress, embedding migration status, integrity coverage |
| `memory_verify`           | Sweep all memories for SHA-256 integrity mismatches. `fix=True` backfills missing hashes            |
| `memory_migrate_embedder` | Switch embedding providers without losing memories. Re-embeds in background; BM25 continues during migration |
| `memory_summarize`        | Manually trigger summarization backfill for memories without summaries                              |
...
|----------------------|--------------------------------------------------------|-----------------|----------------------------------------------------------|
| `"summary"` **(default)** | 1–2 sentence summary generated by Ollama          | ~150 chars      | AI agents — maximum context efficiency                   |
| `"chunk"`            | The matched excerpt from the source memory             | ~200 chars      | When you need the exact passage that matched             |
| `"full"`             | Original content, unmodified                           | 200–50,000 chars | Export, debugging, or when full fidelity is required    |

Summaries are generated asynchronously by a background daemon using your configured Ollama model (`ENGRAM_SUMMARIZE_MODEL`, default: `llama3.2`). New memories fall back to `matched_chunk` until summarized — typically within 30 seconds of storage.

At scale, `detail="summary"` reduces context window consumption by ~13x compared to `detail="full"`.
...
**Switching embedding providers:** Once a project stores its first embedding, Engram locks the project to that model. To switch providers, call `memory_migrate_embedder(project="my-app", new_embedder="openai/text-embedding-3-small")`. This nulls all existing embeddings and triggers background re-embedding with the new provider. Vector search degrades to BM25+recency during migration — typically a few minutes for large projects. You do not need to delete or re-ingest memories.
...
| `ENGRAM_SUMMARIZE_ENABLED` | `true`           | Enable background memory summarization                     |
| `ENGRAM_SUMMARIZE_MODEL`   | `llama3.2`       | Ollama model for summarization                             |
...
```python
# 1. Find out where the last agent left off — full fidelity, not a summary
...
**Vector dimension lock.** Once a project records its first embedding, you can't mix vectors from different models in the same project — Engram refuses rather than silently corrupting search. Use `memory_migrate_embedder` to switch providers; it re-embeds everything with the new model in the background.
